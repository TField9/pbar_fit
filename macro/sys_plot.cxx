...
void sys_plot(){
    ...
    const char* names[] = {
        "RooComb","TFF","IWLS","DA","JSC","asy","PG","URComb","URFit", // 添加PG在asy后
        "KP0.7","KP0.9","KP1.0","KP1.1","KP1.3"
    };
    int n = size.size();
    const int base_methods = 9; // 原为8，增加PG后改为9
    ...
    for(int i = 0; i < n; ++i){
        // 输入文件路径
        TString fn_binned_roocomb = Form("/data03/tianye/pbar/root_paper/sys_result/binned_roocomb_seq0001_npi%04d.root", size[i]);
        TString fn_binned_TFF     = Form("/data03/tianye/pbar/root_paper/sys_result/binned_TFF_seq0001_npi%04d.root",     size[i]);
        TString fn_binned_IWLS    = Form("/data03/tianye/pbar/root_paper/sys_result/binned_IWLS_seq0001_npi%04d.root",   size[i]);
        TString fn_binned_OM      = Form("/data03/tianye/pbar/root_paper/sys_result/binned_OM_seq0001_npi%04d.root",     size[i]);
        TString fn_un_roocomb     = Form("/data03/tianye/pbar/root_paper/sys_result/unbinned_roocomb_seq0001_npi%04d.root", size[i]);
        TString fn_un_roofit      = Form("/data03/tianye/pbar/root_paper/sys_result/unbinned_roofit_seq0001_npi%04d.root", size[i]);
        TString fn_un_keypdf      = Form("/data03/tianye/pbar/root_paper/sys_result/unbinned_rookeyspdf_diffwidth_seq0001_npi%04d.root", size[i]);
        TString fn_binned_pg      = Form("/data03/tianye/pbar/root_paper/sys_result/binned_pg_seq0001_npi%04d.root", size[i]);
        ...
        if(gSystem->AccessPathName(fn_binned_roocomb) ||
           ...
           gSystem->AccessPathName(fn_binned_pg)) {   // 添加PG文件检查
        ...
        TFile *f_pg      = new TFile(fn_binned_pg,      "READ"); // 添加PG文件
        ...
        TTree *t_pg      = (TTree*)f_pg     ->Get("fitParamsTree"); // 添加PG树
        
        if(!t_roocomb||!t_TFF||!t_IWLS||!t_OM||!t_unRC||!t_unRF||!t_keypdf||!t_pg){
            std::cout<<"缺失树，N="<<size[i]<<std::endl;
            for(auto f:{f_roocomb,f_TFF,f_IWLS,f_OM,f_unRC,f_unRF,f_keypdf,f_pg}) f->Close();
            continue;
        }

        // 参数容器
        std::vector<double>* p_roocomb = new std::vector<double>(8);
        std::vector<double>* p_TFF     = new std::vector<double>(8);
        std::vector<double>* p_IWLS    = new std::vector<double>(8);
        Double_t pDA[2]={0}, pJSC[2]={0}, pasy[2]={0}, pDA1[2]={0}, pJSC1[2]={0}, pasy1[2]={0};
        std::vector<double>* p_unRC    = new std::vector<double>(8);
        std::vector<double>* p_unRF    = new std::vector<double>(8);
        std::vector<double>* p_pg      = new std::vector<double>(8); // PG参数容器
        ...
        t_pg     ->SetBranchAddress("nsig", &p_pg); // 绑定PG分支
        ...
        for(int j = 0; j < nSys; ++j){
            ...
            for(int k=0; k<entries; ++k){
                ...
                t_OM     ->GetEntry(k);
                t_keypdf ->GetEntry(k);
                t_pg     ->GetEntry(k); // 读取PG数据

                // 前九种
                hs[0]->Fill((*p_roocomb)[j]);      // RooComb
                hs[1]->Fill((*p_TFF)[j]*((j==0||j==2)?400:250)); // TFF
                hs[2]->Fill((*p_IWLS)[j]);         // IWLS
                
                // DA, JSC, asy
                double *dA = (j<2? pDA:pDA1), *jsc=(j<2? pJSC:pJSC1), *asyarr=(j<2? pasy:pasy1);
                int idx = (j<2? j: j-2);
                hs[3]->Fill(dA[idx]);  // DA
                hs[4]->Fill(jsc[idx]); // JSC
                hs[5]->Fill(asyarr[idx]); // asy
                
                hs[6]->Fill((*p_pg)[j]);       // PG (新增位置)
                hs[7]->Fill((*p_unRC)[j]);     // URComb
                hs[8]->Fill((*p_unRF)[j]);     // URFit
        ...
        delete p_unRC; delete p_unRF; delete p_keypdf; delete p_pg;
        for(auto f:{f_roocomb,f_TFF,f_IWLS,f_OM,f_unRC,f_unRF,f_keypdf,f_pg}) f->Close();
    }
    ...
    Int_t cols[] = {
        kRed,kBlue,kMagenta,kCyan,kViolet,kGreen,kOrange+1,kPink,kYellow,
