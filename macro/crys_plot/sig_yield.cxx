...
void sig_yield(){
    ...
    gStyle->SetErrorX(0);
    c1->Divide(3,4);
    c1->Print("/data03/tianye/pbar/macro/unbinned_fit/bin1/macro/fixneg/macro/Nmc100/pdf/bias_plot8000_crys.pdf[");
    ...
    for (int i : size){ 
            ...
            TFile *f=new TFile(Form("/data03/tianye/pbar/root/bin25_Nmc_crys/binned_roocomb_seq0001_npi%04d.root",i),"read");
            ...
            TFile *f3=new TFile(Form("/data03/tianye/pbar/root/bin25_Nmc_crys/binned_IWLS1_seq0001_npi%04d.root",i),"read");
            TFile *f4=new TFile(Form("/data03/tianye/pbar/root/bin25_Nmc_test/unbinned_roocomb_seq0001_npi%04d.root",i),"read");
            TFile *f5=new TFile(Form("/data03/tianye/pbar/root/bin25_Nmc_crys/unbinned_roofit_seq0001_npi%04d.root",i),"read");
            ...
            TH1D* bin20_roocomb = new TH1D("bin20_roocomb", "bin20_roocomb", 100, 160, 240);
            TH1D* bin100_roocomb = new TH1D("bin100_roocomb", "bin100_roocomb", 100, 160, 240);
            // TH1D* bin20_TFF = new TH1D("bin20_TFF", "bin20_TFF", 100, 160, 240);
            // TH1D* bin100_TFF = new TH1D("bin100_TFF", "bin100_TFF", 100, 160, 240);
            // TH1D* bin20_SPD = new TH1D("bin20_SPD", "bin20_paper", 100, 160, 240);
            // TH1D* bin100_SPD = new TH1D("bin100_SPD", "bin100_paper", 100, 160, 240);
            TH1D* bin20_IWLS = new TH1D("bin20_IWLS", "bin20_IWLS", 100, 160, 240);
            TH1D* bin100_IWLS = new TH1D("bin100_IWLS", "bin100_IWLS", 100, 160, 240);
            TH1D* unbinned_comb = new TH1D("unbinned_comb", "unbinned_comb", 100, 160, 240);
            TH1D* unbinned_roofit = new TH1D("unbinned_roofit", "unbinned_roofit", 100, 160, 240);
            ...
            TH1D* unbinned_roofit_pull= new TH1D("unbinned_roofit_pull", "unbinned_roofit_pull", 200, -3, 3);
                ...
                all_fit.push_back({gaussFit1, gaussFit2,  gaussFit7, gaussFit8, gaussFit9, gaussFit10});
    ...
    c2->SetLogx();
    c2->SetGridy();
    const char *titles[6] = {"bin20_roocomb", "bin100_roocomb", "bin20_IWLS", "bin100_IWLS", "unbinned_comb", "unbinned_roofit"};
    ...
    legend->Draw();
    c2->Update();
    c2->Print("/data03/tianye/pbar/macro/unbinned_fit/bin1/macro/fixneg/macro/Nmc100/pdf/bias_plot8000_crys.pdf");
    ...
    for (size_t j = 0; j < size.size(); ++j) {
        ...
        c3->Update();
        c3->Print("/data03/tianye/pbar/macro/unbinned_fit/bin1/macro/fixneg/macro/Nmc100/pdf/bias_plot8000_crys.pdf");
        if (j == size.size() - 1) {
            c3->Print("/data03/tianye/pbar/macro/unbinned_fit/bin1/macro/fixneg/macro/Nmc100/pdf/bias_plot8000_crys.pdf]");
        }
    }
